class User < ApplicationRecord
  ...
  validates :last_name, presence: { message: 'お名前(全角)を入力してください' }, format: { with: /\A[ぁ-んァ-ヶ一-龥々ー]+\z/, message: 'は全角ひらがな、カタカナ、漢字で入力してください' }
  validates :first_name, presence: { message: 'お名前(全角)を入力してください' }, format: { with: /\A[ぁ-んァ-ヶ一-龥々ー]+\z/, message: 'は全角ひらがな、カタカナ、漢字で入力してください' }  
